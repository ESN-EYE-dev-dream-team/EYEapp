...
const _flatMap = _interopRequireDefault(require('lodash/flatMap'));
...
const GoogleSheetsProvider = (function(_Component) {
    ...
    function GoogleSheetsProvider() {
        ...
        //         )
        //         .concat(process.env.REACT_APP_GOOGLE_SHEETS_API_KEY);
        // });

        _defineProperty(_assertThisInitialized(_this), 'getUrl', function() {
            return 'https://sheets.googleapis.com/v4/spreadsheets/'
                .concat(
                    '1jSCZTQmEyb9oUscoOQz9hSbQ_U9x6qjoAZ9uAGpjfO4',
                    '?includeGridData=true&fields=sheets(data%2FrowData%2Fvalues%2FformattedValue%2Cproperties%2Ftitle)&key=',
                )
                .concat('AIzaSyAPmCnDLhm3xgQxypFi4g7Db6uUznjGai8');
        });

        _defineProperty(_assertThisInitialized(_this), 'processData', function(_ref) {
            const sheets = _ref.sheets;
            let result = {};
            sheets.forEach(function(sheet) {
                ...
                if (data[0].rowData) {
                    let _data$0$rowData = _toArray(data[0].rowData),
                                ...
                                headerRow.forEach(function(value, index) {
                                    result = _objectSpread(
                                        _defineProperty(
                                            {},
                                            value,
                                            record.values[index] ? record.values[index].formattedValue : null,
                                        ),
                                        result,
                                    );
                                });
...
const _default = GoogleSheetsProvider;
